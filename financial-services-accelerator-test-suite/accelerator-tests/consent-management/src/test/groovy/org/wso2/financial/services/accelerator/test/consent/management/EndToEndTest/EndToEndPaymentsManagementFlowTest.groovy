 ...
 */
class EndToEndPaymentsManagementFlowTest extends FSConnectorTest {
    ...
    void setUp() {
        ...
        initiationPayload = PaymentRequestPayloads.initiationPayload
    }
...
//        List<ConnectorTestConstants.ApiScope> scopeList = ConsentMgtTestUtils.getApiScopesForConsentType(map.get("consentType"))
    ...
    void "Verify Create the consent with valid inputs"() {
        ...
        Assert.assertEquals(TestUtil.parseResponseBody(consentResponse, ConnectorTestConstants.SELF_LINK),
                "https://api.alphabank.com/open-banking/v3.1/pisp/domestic-payment-consents" + "/" + consentId)
    ...
    void "Validate Retrieval on valid account for requestUri"() {
        ...
        validationPayload = PaymentRequestPayloads.buildPaymentValidationPayload(accessToken, userId, consentId,
                ConnectorTestConstants.PAYMENT_SUBMISSION_PATH)
